...
def LogReg(X_train, Y_train, X_test, Y_true):
    clf = LogisticRegression()
    #Tuning hyperparameters from : https://towardsdatascience.com/hyperparameter-tuning-in-python-21a76794a1f7
    parameters = [{'solver': ['newton-cg', 'lbfgs', 'liblinear', 'sag', 'saga']},
                  {'penalty':['none', 'elasticnet', 'l1', 'l2']},
                  {'C':[0.001, 0.01, 0.1, 1, 10, 100]}]

    clf = GridSearchCV(estimator=clf, param_grid=parameters, cv=12, n_jobs=-1, scoring='accuracy')
    ...
    print()
    #print(accuracy_score(Y_true, Y_test_log))
    print()
    #print(X_testLog[X_testLog.iloc[:, -2] == 1])

    X_testLog.to_pickle('X_testLog.pickle')

## Not used
def MLP(X_train, Y_train, X_test, Y_True):
    ...
    print(accuracy_score(Y_True, Y_test_MLP))
    print()
    print(X_testMLP[X_testMLP.iloc[:, -2] == 1])
